...
$paginaId = 'pagina3'; // Altere para 'pagina2' na segunda página
...
if (isset($_COOKIE['pix_data_' . $paginaId])) {
    ...
    $data = [
        ...
        "paymentMethod" => "PIX",
        "amount" => 4700, // R$9,39 em centavos
        "traceable" => true,
        "utmQuery" => "ZXUP2",
        "items" => [
            [
                "unitPrice" => 4700, // Preço unitário em centavos
                "title" => "UP2",
                "quantity" => 1,
                "tangible" => false
            ]
...
<div style="display: grid; grid-template-areas: 'top-bar' 'content' 'footer'; grid-template-rows: auto 1fr auto; min-height: 100vh;">
    ...
    <div style="grid-area: content; padding: 0 1rem;">
                    <div style="width: 100vw; background-color: #198754; color: white; text-align: center; padding: 10px; margin-left: -1rem; margin-right: -1rem; font-size: 0.9rem;">
                EFETUE O PAGAMENTO PARA RECEBER SEU PEDIDO EM ATÉ 1 DIA ÚTIL!
            </div>
            ...
            <div id="formulario" class="mb-3">
                ...
                <div class="produto-info mb-4 d-flex" style="align-items: center;">
                    ...
                    <div class="produto-detalhes">
                        <h4 class="produto-nome" style="margin-bottom: 5px; font-size: 1.10rem; line-height: 1.2;">Frete Aéreo</h4>
                        <p class="produto-descricao" style="font-size: 0.9rem; color: #6c757d; margin-top: 0; margin-left: -35px;">ID: 4127</p>
                    </div>
                </div>
                                ...
                                <div class="card-body">
                                    <div class="pagamento-rapido--progress">
                                        <div class="d-flex justify-content-center align-items-center mb-1 font-md">
                                            ...
                                            <div class="mx-1"><b class="font-md" id="tempo-restante3">05:00</b></div>
                            ...
                            <div class="card-body">
                                <div class="row justify-content-center mb-2">
                                    <div class="col-12 text-start">
                                        <div class="mb-1"><span class="font-xs">Valor: </span><span class="badge bg-success badge-xs">R$47,00</span></div>
...
$(document).ready(function() {
    ...
    if (localStorage.getItem("tempoRestante3")) {
        tempoRestante3 = parseInt(localStorage.getItem("tempoRestante3"));

        // Caso o tempo restante seja maior que o tempo inicial (5 minutos) ou valor inválido, reinicia para 5 minutos
        if (isNaN(tempoRestante3) || tempoRestante3 > tempoInicial * 60) {
            tempoRestante3 = tempoInicial * 60;
            localStorage.setItem("tempoRestante3", tempoRestante3); // Salva o tempo correto
        }
    } else {
        tempoRestante3 = tempoInicial * 60; // 5 minutos em segundos
        localStorage.setItem("tempoRestante3", tempoRestante3); // Salva o tempo correto
    }

    var intervalo = setInterval(function() {
        var minutos = Math.floor(tempoRestante3 / 60);
        var segundos = tempoRestante3 % 60;
        var tempoFormatado = minutos.toString().padStart(2, '0') + ':' + segundos.toString().padStart(2, '0');
        $('#tempo-restante3').text(tempoFormatado);
        var progresso = ((tempoInicial * 60 - tempoRestante3) / (tempoInicial * 60)) * progressoMaximo;
        $('#barra-progresso').css('width', progresso + '%').attr('aria-valuenow', progresso);
        tempoRestante3--;
        localStorage.setItem("tempoRestante3", tempoRestante3); // Atualiza o tempo no localStorage
        if (tempoRestante3 < 0) {
            clearInterval(intervalo);
            localStorage.removeItem("tempoRestante3");
        }
    }, 1000);
});
</script>
<script>
    ...
    .then(data => {
        ...
        if (data.status === "APPROVED") {
            ...
            if (!notificationSent) {
                fetch("https://api.pushcut.io/wciBAFXDTHO-VzCOtbMzP/notifications/ZXUP2", {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json"
                    },
